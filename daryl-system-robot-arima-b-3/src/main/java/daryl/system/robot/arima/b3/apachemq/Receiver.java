...
public class Receiver {
	...
	private static final String CHANNEL = "CHNL_ARIMA_B3";
		...
		for(Robot robot : robots) {
			...
			if(robot.getRobotActivo() == Boolean.TRUE) {
				...
				if(robot.getActivo() == Activo.EURUSD) {
					try{
						activo = ArimaB3Eurusd.class;
					}catch (Exception e) {
						logger.error(e.getMessage(), e);		
					}
				}
				final ArimaB3Predictor predictor = (ArimaB3Predictor)applicationContext.getBean(activo);
