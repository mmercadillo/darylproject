	...
	protected Double calcularPrediccion(Robot bot) throws IOException {
		...
		if(annConfig != null) {
			...
			if(ann != null) {		
				...
				List<Historico> historico = historicoRepository.findAllByTimeframeAndActivoOrderByFechaHoraDesc(bot.getTimeframe(), bot.getActivo(), PageRequest.of(0,  100));
				...
				prediccion = prediccionANN(ann, datos.subList(datos.size()-20, datos.size()));
